...
import { makeId, MessageList, sessionID } from "./MessageList"
      ...
      >
        ...
        {message.rating !== 3 && (
          <div className="absolute top-0 right-0 block group-hover:hidden">
            {message.rating < 3 && <Frown className="text-red-300/30" size={24} />}
            {message.rating > 3 && <Smile className="text-green-300/30" size={24} />}
          </div>
        )}
...
export namespace Message {
  ...
  export const sendPromptMessage = async (prompt: string, modifiers?: string) => {
